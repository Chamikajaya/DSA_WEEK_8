...
struct HashTable{
    ...
    int hashfunc(string user_name){
        int sum = 0;
        int hash = 0;
        // add up ASCII values of each character in the string
    ...
    void insert(string user_name, string user_password){
        ...
        if(empty){
            // Slot is empty, insert the new element
            password[hash] = user_password;
            cout << "User " << user_name << " added\n";
        } else {
            // Collision, find next empty slot and insert the new element
            int i = (hash + 1) % MAX_LENGTH;
            while(i != hash && !is_slot_empty(i)) {
                i = (i + 1) % MAX_LENGTH;
            }
            if(i == hash) {
                // Table is full
                cout << "Unable to add user " << user_name << ", hash table is full\n";
            } else {
                // Insert new element at empty slot
